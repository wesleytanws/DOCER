...
evaluate_repo() {
    ...
    printf '%s\n' "page_id,rev_id,code_element,count" > "$output_dir/tmp/$repo_name/matches.csv"
    printf '%s\n' "page_id,rev_id,code_element,file_name,line_number" > "$output_dir/tmp/$repo_name/sources.csv"
    printf '%s\n' "page_id,page_type,page_name" > "$output_dir/tmp/$repo_name/pages.csv"
    printf '%s\n' "page_id,rev_id,rev_SHA,rev_timestamp,doc_SHA,doc_timestamp" > "$output_dir/tmp/$repo_name/revisions.csv"
...
evaluate_page() {
    ...
    printf '%s,%s,"%s"\n' "$page_id" "${page_dir##*/}" "${page_name//\"/\"\"}" >> "$output_dir/tmp/$repo_name/pages.csv"

    # Get when the page was last updated
    snapshot_timestamp="$(git -C "$page_dir" log -1 --first-parent --pretty=format:%ct HEAD -- "$page_name")"
    snapshot_SHA="$(tail -1 <(git -C "$repo_dir" rev-list --first-parent --max-age="$snapshot_timestamp" HEAD 2> /dev/null))"
...
evaluate_revision() {
    ...
    ) >> "$output_dir/tmp/$repo_name/sources.csv"
    ...
    ) >> "$output_dir/tmp/$repo_name/matches.csv"
